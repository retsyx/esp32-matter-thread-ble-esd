...
int8_t BMA400::beginI2C(uint8_t address, i2c_port_t port)
{
...
int8_t BMA400::beginSPI(uint8_t csPin, uint32_t clockFrequency)
{
    ...
    return begin();
    #endif
    ESP_LOGE(TAG, "SPI interface not implemented");
...
BMA400_INTF_RET_TYPE BMA400::readRegistersI2C(uint8_t regAddress, uint8_t* dataBuffer, uint32_t numBytes, BMA400_InterfaceData* interfaceData)
{
    ...
    i2c_master_write_byte(cmd, (interfaceData->i2cAddress << 1) | I2C_MASTER_WRITE, true /* enable_ack */);
    i2c_master_write_byte(cmd, regAddress, true);
    i2c_master_stop(cmd);
    auto err = i2c_master_cmd_begin(interfaceData->i2c_port, cmd, pdMS_TO_TICKS(1000));
    i2c_cmd_link_delete(cmd);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed read request, err: %d", err);
        return BMA400_E_COM_FAIL;
    }
    ...
    cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (interfaceData->i2cAddress << 1) | I2C_MASTER_READ, true /* enable_ack */);
    i2c_master_read(cmd, dataBuffer, numBytes, I2C_MASTER_LAST_NACK);
    i2c_master_stop(cmd);
    err = i2c_master_cmd_begin(interfaceData->i2c_port, cmd, pdMS_TO_TICKS(1000));
    i2c_cmd_link_delete(cmd);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed read, err: %d", err);
        return BMA400_E_COM_FAIL;
    }
...
BMA400_INTF_RET_TYPE BMA400::writeRegistersI2C(uint8_t regAddress, const uint8_t* dataBuffer, uint32_t numBytes, BMA400_InterfaceData* interfaceData)
{
    ...
    i2c_master_write_byte(cmd, (interfaceData->i2cAddress << 1) | I2C_MASTER_WRITE, true /* enable_ack */);
    i2c_master_write_byte(cmd, regAddress, true);
    i2c_master_write(cmd, dataBuffer, numBytes, true);
    i2c_master_stop(cmd);
    auto err = i2c_master_cmd_begin(interfaceData->i2c_port, cmd, pdMS_TO_TICKS(1000));
    i2c_cmd_link_delete(cmd);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Failed write, err: %d", err);
        return BMA400_E_COM_FAIL;
    }
